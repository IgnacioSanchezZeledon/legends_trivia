# views/levels_view.py — Mapa de niveles en Canvas + iconos música/sonido (abajo-izquierda)
import math
import os
import time
...
class LevelsView(ttk.Frame):
    ...
    # Paleta de nodos
    PRIMARY        = "#2B6EA6"  # normal
    PRIMARY_HOVER  = "#3A84C2"  # hover
    PRIMARY_BORDER = "#1F5A86"  # borde

    LOCK_FILL   = "#9DA3A6"
    LOCK_BORDER = "#70757A"

    # Imagen por defecto (se resolverá con resource_path en __init__)
    DEFAULT_BG = "assets/images/levels_spooky.png"

    # Posiciones fijas para 6 niveles (x_rel, y_rel)
    NODE_POS = [
        (0.14, 0.38),
        (0.23, 0.65),
        (0.58, 0.74),
    ...
    # Camino poligonal (si se necesitan más puntos se remuestrea)
    PATH_POLY = [(0.20, 0.82), (0.35, 0.70), (0.50, 0.60), (0.62, 0.50), (0.74, 0.38), (0.86, 0.22)]

    # Iconos música/sonido
    _TOP_ICONS_H = 28
    _TOP_ICONS_GAP = 10
    _TOP_ICONS_PAD = 12
    ...
    def __init__(self, parent, controller, progress_model, total_levels, switch_view, bg_path=None,
        ...
        """Inicializa Canvas, fondo, header, nodos e íconos de audio; registra eventos y hotkeys."""
        ...
        # Resolver ruta del fondo SIEMPRE a absoluta (funciona en exe y en desarrollo)
    ...
    # ================= API pública =================
    def refresh(self):
        """Actualiza desbloqueo, etiqueta 'n/3' y aspecto."""
        for nd in self.nodes:
            ...
            # centro: número o candado
            self.canvas.itemconfigure(
                nd["num_item"],
                text=(str(n) if unlocked else "🔒"),
                font=("Mikado Ultra", 20, "bold") if unlocked else ("Mikado Ultra", 18)
            )
    ...
    def _build_nodes(self):
        ...
        for i in range(self.total):
            ...
            img_norm  = self._token_img(self.NODE_D, fill=self.PRIMARY,       border=self.PRIMARY_BORDER)
            img_hover = self._token_img(self.NODE_D, fill=self.PRIMARY_HOVER, border=self.PRIMARY_BORDER)
            img_lock  = self._token_img(self.NODE_D, fill=self.LOCK_FILL,     border=self.LOCK_BORDER)

            img_item = self.canvas.create_image(0, 0, image=img_norm, anchor="center")

            center_text = str(n) if unlocked else "LOCK"
            center_font = ("Mikado Ultra", 20, "bold") if unlocked else ("Mikado Ultra", 18)
            num_item = self.canvas.create_text(0, 0, text=center_text, fill="#ffffff",
                                               font=center_font, anchor="center")

            label = (f"{stars}/3" if unlocked else "")
            label_item = self.canvas.create_text(0, 0, text=label, fill="#ffffff",
                                                 font=("Mikado Ultra", 12), anchor="n")
        ...
        # Visual inicial
        for nd in self.nodes:
            self._apply_node_visual(nd, False)
    ...
    def _layout_all(self):
        ...
        # Header bar
        key = (w, self.BAR_H, self.TOPBAR, 220)
        if key not in self._hdr_cache:
            img = Image.new("RGBA", (w, self.BAR_H), (*self._hex(self.TOPBAR), 220))
        ...
        self.canvas.coords(self._title_txt, w // 2, self.BAR_H // 2)
        if self._back_btn:
            self.canvas.coords(self._back_btn["img_item"], 12 + 60, self.BAR_H // 2)
            self.canvas.coords(self._back_btn["txt_item"], 12 + 60, self.BAR_H // 2)

        # Nodos
        for nd in self.nodes:
            x_rel, y_rel = nd["pos"]
            cx = int(x_rel * w)
            cy = int(y_rel * (h - self.BAR_H)) + self.BAR_H
            self.canvas.coords(nd["img_item"], cx, cy)
            self.canvas.coords(nd["num_item"], cx, cy)
            self.canvas.coords(nd["label_item"], cx, cy + self.NODE_D // 2 + 4)

        # Íconos música/sonido
    ...
    def _token_img(self, d, fill="#2B6EA6", border="#1F5A86"):
        key = ("token", d, fill, border)
        if key in self._img_cache:
            return self._img_cache[key]
        aa = 4
        D = d * aa
        base = Image.new("RGBA", (D, D + self.SH_OFF * aa), (0, 0, 0, 0))
        # sombra
        sh = Image.new("RGBA", (D, D), (0, 0, 0, 0))
        ImageDraw.Draw(sh).ellipse([0, 0, D - 1, D - 1], fill=(0, 0, 0, 120))
        sh = sh.filter(ImageFilter.GaussianBlur(self.SH_BLUR * aa))
        base.alpha_composite(sh, (0, self.SH_OFF * aa // 2))
        # círculo
        ImageDraw.Draw(base).ellipse([0, 0, D - 1, D - 1], fill=fill, outline=border, width=8 * aa)
        base = base.resize((d, d + self.SH_OFF), Image.Resampling.LANCZOS)
    ...
    def _create_rect_button(self, text, command, width, height, r,
        ...
        txt_item = self.canvas.create_text(0, 0, text=text, fill=text_color,
                                           font=("Mikado Ultra", 16), anchor="center")
        btn = {"img_item": img_item, "txt_item": txt_item, "img_norm": img_norm, "img_hover": img_hover, "cmd": command}
    ...
    def _print_norm_xy(self, e):
        ...
        y_rel = round((e.y - self.BAR_H) / max(1, (h - self.BAR_H)), 4)
        print(f"click @ ({x_rel}, {y_rel})  # add to NODE_POS")

    # ========= Íconos de música/sonido =========
    ...
    def _load_sound_icons(self):
        """Carga, escala y tinta íconos de música/SFX desde assets empaquetado."""
        try:
            # <<< CLAVE: resolver ruta de icons con assets_path >>>
